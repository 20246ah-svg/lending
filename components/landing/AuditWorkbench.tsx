    ...
    <section id="report-view" className="py-20 px-4 sm:px-8 max-w-6xl mx-auto z-10 relative">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-950/90 shadow-2xl overflow-hidden backdrop-blur-xl">
        ...
        <div className="px-6 py-4 bg-zinc-900/70 border-b border-zinc-800 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-xs text-zinc-300 font-bold ml-2">
              {lang === "ru" ? "ОТЧЕТ АУДИТА:" : "AUDIT REPORT:"} {report.repoName}
            </span>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-zinc-800 text-zinc-400">
              {report.filesScanned} {lang === "ru" ? "файлов проверено" : "files scanned"}
            </span>
          </div>
        ...
        <div className="flex border-b border-zinc-800 text-xs font-mono bg-zinc-900/40 px-6 gap-2 overflow-x-auto">
          {[
            { id: "antipatterns", label: lang === "ru" ? `Точки отказа (${report.antipatterns.length})` : `Failure Points (${report.antipatterns.length})` },
            { id: "god-files", label: lang === "ru" ? `God-компоненты (${report.godComponents.length})` : `God Components (${report.godComponents.length})` },
            { id: "prompts", label: lang === "ru" ? `Хирургические промпты (${report.refactorSteps.length})` : `Surgical Prompts (${report.refactorSteps.length})` },
          ].map((tab) => (
            <button
              key={tab.id}
              ...
              {report.refactorSteps.map((step, idx) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    >
                      {copiedPromptIdx === idx ? <CheckIcon size={13} /> : <CopyIcon size={13} />}
