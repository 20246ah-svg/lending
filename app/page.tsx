...
import { track, extractAttribution } from "@/lib/analytics";
...
export default function Home() {
  ...
  useEffect(() => {
    track("landed", {
      ...
      ...extractAttribution(typeof window !== "undefined" ? window.location.search : ""),
    });
  ...
  const runAudit = async (payload: AuditRequestBody) => {
    ...
    try {
      ...
      if (!res.ok || json.error) {
        ...
        track("scan_failed", { mode, status: res.status });
      } else if (json.data) {
      ...
      track("scan_failed", { mode, status: 0 });
      setErrorMessage(
        lang === "ru"
